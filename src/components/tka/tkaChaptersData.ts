...
// Helper to generate a full set of 30 questions for a session if needed
// Ensures that every chapter has proportional representation in the session
export function getQuestionsForSubject(
  subject: TkaSubject,
  targetCount: number = 30,
  allBankQuestions: TkaQuestion[] = INITIAL_TKA_QUESTIONS
): TkaQuestion[] {
  const subjectBank = allBankQuestions.filter((q) => q.subject === subject);
  if (subjectBank.length === 0) return [];

  // If we already have enough, take targetCount
  if (subjectBank.length >= targetCount) {
    return subjectBank.slice(0, targetCount);
  }

  // Otherwise, clone and vary parameters so the user gets 30 high-quality questions
  const result: TkaQuestion[] = [...subjectBank];
  let cycle = 1;
  while (result.length < targetCount) {
    const baseQ = subjectBank[(result.length - subjectBank.length) % subjectBank.length];
    const newQ: TkaQuestion = {
      ...baseQ,
      id: `${baseQ.id}_v${cycle}_${result.length}`,
      question: `[Variasi ${cycle + 1}] ${baseQ.question}`,
    };
    result.push(newQ);
    if (result.length % subjectBank.length === 0) {
      cycle += 1;
    }
  }
