...
import { TkaQuestion, TkaSubject, TkaQuestionType, TkaSkill } from './tkaTypes';
...
type SubTab = 'LIST' | 'PDF_UPLOAD' | 'SMART_TEXT' | 'EXCEL_UPLOAD';
...
export const AdminTkaManagerView: React.FC<AdminTkaManagerViewProps> = ({ onLogActivity }) => {
  ...
  }, [questions, selectedSubjectFilter, selectedTypeFilter, searchQuery]);
  ...
  const handleResetDefault = () => {
    if (
      window.confirm(
        'PERINGATAN: Anda akan mengembalikan Bank Soal ke susunan standar awal (termasuk paket SMPN 1 Bojongsari & paket standar GO). Lanjutkan?'
      )
    ) {
      resetTkaQuestionsToDefault();
      refreshQuestions();
  ...
  const parseQuestionsFromRawText = (
    ...
    try {
      ...
      questionBlocks.forEach((block, idx) => {
        ...
        const theKingMatch = fullContent.match(/(?:The King|Solusi Kilat|Trik GO|Rumus Cepat)\s*[\:\-]\s*([\s\S]*?)(?=\n\s*(?:Solusi|Kunci|\d+[\.\)])|$)/i);
        if (theKingMatch) {
      ...
      {activeSubTab === 'LIST' && (
        <div className="space-y-3">
          ...
          <div className="neu-flat rounded-2xl p-3.5 bg-[#E7EBF5] border border-white/70 flex flex-col md:flex-row items-center gap-3">
            {/* Search */}
            <div className="neu-inset rounded-xl p-2 flex items-center gap-2 border border-white/50 w-full md:w-80">
        ...
        <div className="neu-flat rounded-2xl p-4 sm:p-5 bg-[#E7EBF5] border border-white/70 space-y-4">
          <div className="space-y-1">
            <h5 className="font-heading font-bold text-sm text-[#1F3A5F] flex items-center gap-1.5">
              ...
              <span>Upload & Ekstraksi Dokumen PDF Soal TKA</span>
            </h5>
            <p className="text-xs text-gray-600 leading-relaxed">
              Unggah file PDF bank soal Anda (misal lembar try out, modul bimbel, latihan semester). Sistem akan membaca dan mengekstrak teks seluruh halaman secara otomatis, merapikan nomor soal & pilihan A-D, mendeteksi tipe soal PG vs PGK, serta membaca trik The King.
            </p>
          </div>
        ...
        <div className="neu-flat rounded-2xl p-4 sm:p-5 bg-[#E7EBF5] border border-white/70 space-y-4">
          <div className="space-y-1">
            <h5 className="font-heading font-bold text-sm text-[#1F3A5F] flex items-center gap-1.5">
              ...
              <span>Import via Spreadsheet Excel / CSV (.xlsx, .csv)</span>
            </h5>
            <p className="text-xs text-gray-600 leading-relaxed">
              Unggah file spreadsheet soal Anda. Anda dapat mengunggah 1 file Excel lengkap yang berisi kolom soal dan kunci jawaban, ATAU mengunggah file kunci jawaban terpisah.
            </p>
          </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[11px] font-bold text-amber-900 flex items-center gap-1 mb-1">
                    ...
                    <span>Metode THE KING (Trik Kilat GO):</span>
                  </label>
                  <textarea
                    rows={2}
                    value={formTheKing}
